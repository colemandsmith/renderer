...
RenderObject::RenderObject(Model *model, Material *material, std::string name,
                           ...
                           GLfloat xRotation, GLfloat yRotation,
                           GLfloat zRotation) {
  this->name = name;
  this->model = model;
  this->material = material;
...
glm::mat4 RenderObject::GetModelMatrix() {
  ...
  if (xRotation != 0.0f) {
    modelMatrix = glm::rotate(modelMatrix, glm::radians(xRotation),
                              glm::vec3(1.0f, 0.0f, 0.0f));
  }
  if (yRotation != 0.0f) {
    modelMatrix = glm::rotate(modelMatrix, glm::radians(yRotation),
                              glm::vec3(0.0f, 1.0f, 0.0f));
  }
  if (zRotation != 0.0f) {
    modelMatrix = glm::rotate(modelMatrix, glm::radians(zRotation),
...
void RenderObject::Render(Shader *shader) {
  ...
  if (material) {
    // material->UseMaterial(shader->GetSpecularIntensityLocation(),
    // shader->GetShininessLocation());
  }
  model->RenderModel();
}
